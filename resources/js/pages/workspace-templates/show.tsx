...
export default function ShowWorkspaceTemplate({
    template,
    canDelete,
}: ShowTemplateProps) {
    ...
    const handleDuplicate = () => {
        ...
        router.post(
            `/workspace-templates/${template.id}/duplicate`,
            {},
            {
                preserveScroll: true,
                onSuccess: () => {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
                ...
                {/* Header */}
                <header
                    className={`border-b-4 bg-white dark:bg-slate-900 ${categoryColor.replace('bg-', 'border-')}`}
                >
                    <div className="container mx-auto px-6 py-8">
                        ...
                        <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
                            <div className="flex items-start gap-4">
                                <div
                                    className={`flex h-16 w-16 shrink-0 items-center justify-center ${categoryColor} text-white`}
                                >
                                    <CategoryIcon className="h-8 w-8" />
                                </div>
                                <div>
                                    <div className="flex flex-wrap items-center gap-2">
                                        <h1 className="font-mono text-2xl font-black tracking-tight uppercase">
                                            {template.name}
                                        </h1>
                                        {template.is_public && (
                                            <span className="inline-flex items-center gap-1 border border-slate-200 px-2 py-0.5 font-mono text-[10px] tracking-wide uppercase dark:border-slate-700">
                        ...
                        <div className="space-y-6 lg:col-span-2">
                            ...
                            {template.description && (
                                <section className="border-2 border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-900">
                                    <h2 className="mb-4 font-mono text-xs tracking-wide text-slate-500 uppercase">
                                        Description
                                    </h2>
                                    <p className="font-mono text-sm leading-relaxed whitespace-pre-wrap text-slate-700 dark:text-slate-300">
                                        {template.description}
                                    </p>
                                </section>
                            )}
                            ...
                            <section className="border-2 border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-900">
                                ...
                                <ul className="space-y-3 font-mono text-sm">
                                    {[
                                        ...
                                        {
                                            label: 'Custom Fields',
                                            value: `${template.configuration?.custom_fields?.length || 0} fields`,
                                        },
                                        {
                                            label: 'Tags',
                                            value: `${template.configuration?.tags?.length || 0} tags`,
                                        },
                                        {
                                            label: 'Branding',
                                            value:
                                        ...
                                        {
                                            label: 'Webhooks',
                                            value: `${template.configuration?.webhooks?.length || 0} configured`,
                                        },
                                    ].map((item) => (
                                        <li
                                            key={item.label}
                                        ...
                                        >
                                            <span
                                                className={`h-2 w-2 ${categoryColor}`}
                                            />
                            ...
                            <section className="border-2 border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-900">
                                ...
                                <div className="space-y-3">
                                    <Button
                                        onClick={handleUse}
                                        disabled={isUsing}
                                        className="w-full border-2 border-slate-900 bg-violet-500 font-mono text-xs tracking-wide text-white uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all hover:translate-x-[2px] hover:translate-y-[2px] hover:bg-violet-600 hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
                                    >
                                        {isUsing && (
            ...
            <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
                <DialogContent className="border-2 border-slate-200 dark:border-slate-700">
                    <DialogHeader>
                        <DialogTitle className="font-mono text-lg font-bold uppercase">
                            Delete Template
                        </DialogTitle>
