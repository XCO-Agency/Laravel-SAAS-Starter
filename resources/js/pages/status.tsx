...
const STATUS_CONFIG: Record<string, { label: string; badgeClass: string; icon: typeof CheckCircle2; bannerClass: string }> = {
    operational: {
        label: 'All Systems Operational',
        icon: CheckCircle2,
        badgeClass: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300',
        bannerClass: 'border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-300',
    },
    degraded: {
        label: 'Degraded Performance',
        icon: AlertTriangle,
        badgeClass: 'bg-amber-100 text-amber-800 dark:bg-amber-950/60 dark:text-amber-300',
        bannerClass: 'border-amber-200 bg-amber-50 text-amber-800 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-300',
    },
    outage: {
        label: 'Service Outage',
        icon: AlertTriangle,
        badgeClass: 'bg-red-100 text-red-800 dark:bg-red-950/60 dark:text-red-300',
        bannerClass: 'border-red-200 bg-red-50 text-red-800 dark:border-red-900 dark:bg-red-950/30 dark:text-red-300',
    },
    maintenance: {
        label: 'Scheduled Maintenance',
        icon: Wrench,
        badgeClass: 'bg-blue-100 text-blue-800 dark:bg-blue-950/60 dark:text-blue-300',
        bannerClass: 'border-blue-200 bg-blue-50 text-blue-800 dark:border-blue-900 dark:bg-blue-950/30 dark:text-blue-300',
    },
};
            ...
            <div className="min-h-screen bg-background">
                ...
                <main className="mx-auto max-w-3xl px-4 py-12">
                    <div className="mb-10">
                        ...
                        <p className="mt-2 text-muted-foreground">Current operational status and recent incidents.</p>
                    </div>

                    {/* Overall status banner */}
                    <div className={`mb-8 flex items-center gap-3 rounded-xl border p-4 ${overall.bannerClass}`}>
                    ...
                    {incidents.length === 0 ? (
                        <div className="py-16 text-center text-muted-foreground">
                            ...
                            <p className="mt-1 text-sm">Everything is running smoothly.</p>
                        </div>
                    ) : (
                                    ...
                                    <div key={incident.id} className="rounded-xl border p-4">
                                        <div className="mb-2 flex flex-wrap items-center gap-2">
                                            <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${cfg.badgeClass}`}>
                                                {incident.status === 'maintenance' && <Clock className="h-3 w-3" />}
                                                {incident.status === 'outage' && <AlertTriangle className="h-3 w-3" />}
                                                {incident.status === 'degraded' && <AlertTriangle className="h-3 w-3" />}
                                                {incident.status === 'operational' && <CheckCircle2 className="h-3 w-3" />}
                                                {cfg.label}
                                            </span>
